//
//  WithInvertedUserTableViewCell.swift
//  PracticalTest
//
...
class WithInvertedUserTableViewCell: UITableViewCell {
...
extension WithInvertedUserTableViewCell: UserListTableViewCellProtocol {
    func configUserCellData(_ cellData: UserListTableViewCellModelProtocol?) {
        if let cellData = cellData as? WithInvertedUserTableViewCellViewModel {
            ...
            if let url = URL(string: cellData.objUserInfoModel?.avatarURL ?? "") {
                self.ibProfileImageView.loadImage(url, placeHolderImage: self.placeHolderImage, isInvertedImage: true)
            }
        }
    }
}
